...
class Hub {
  ...
  /// Gets the plants of [username] of the hub [hubname]
  static Future<List<Plant?>> hubPlants(String username, String hubname) async {
    return await BackendConnection.getHubPlants(username, hubname).then(
        (response) => (jsonDecode(response.body) as List<dynamic>).map((e) {
              print(e);
              return e == null
                  ? null
                  : Plant(e["type"], e["name"], e["description"]);
            }).toList());
  }

  /// Returns the number of free  there are in the current hub, given the total amount of [slots]
